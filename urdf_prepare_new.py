...
def main(body_model='smpl', body_model_path='/home/datassd/yuxuan/smpl_model/models'):
    ...
    thres = 0.66
    scale_factor = 0.90
    temp_label = np.argwhere(body_model.lbs_weights>thres)
    for part_idx, (k, v) in enumerate(part_segm.items()):
        v_new = list()
        for i in range(24):
            new_label = temp_label[0][temp_label[1]==i]
            refer_new_label = new_label[0]
            if refer_new_label in v:
                v_new += new_label.tolist()

        v_new = [idx for idx in v if idx in v_new]
        mesh = trimesh.Trimesh(vertices[v_new], process=False)
        _mesh = mesh.convex_hull
        # trimesh.smoothing.filter_laplacian(_mesh)
        # _mesh = mesh.copy()
        
        _CoM = _mesh.center_mass #globol position
        bodyparts_CoM[k] = _CoM
        _mesh.vertices -= _CoM #align the CoM to origin
        _mesh.vertices *= scale_factor
        save_dict[k] = _mesh
        part_mesh = _mesh.copy()
        part_mesh.vertices += _CoM
        parts_mesh.append(part_mesh)

        
    for key, m in save_dict.items():
        m.export('../demo/urdf/body_parts_%s.obj' %key)

    scene = trimesh.Scene(parts_mesh)
    scene.export(('../demo/urdf/body_parts.obj'))
    # scene.show()
