...
def sim_loop(path_dict,floor_known=0): 
    ...
    jointIds_reordered = np.array(jointIds)[rbdl2bullet] #below use rbdl order
    l_kafth_ids, r_kafth_ids=ini.get_knee_ankle_foot_toe_heel_ids_rbdl()

    R, T = ini.get_R_T()  
    params = ini.get_params()
    con_j_idx_bullet=ini.get_con_j_idx_bullet() #actually rbdl idx
    ...
    for count in range(n_frames):
        ...
        p.resetBasePositionAndOrientation(id_robot, [q[0], q[1], q[2]], p.getQuaternionFromEuler([angle[2], angle[1], angle[0]])) #'ZYX'
        p.stepSimulation()
